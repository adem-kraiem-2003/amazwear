...
function DesktopProductGrid({
  ...
  return (
    <motion.section
      className="grid grid-cols-2 lg:grid-cols-3 gap-x-gutter gap-y-12"
        ...
        >
          <div className="relative w-full aspect-4-5 bg-surface-container-low overflow-hidden">
            ...
            <Link href={`/product/${p.id}`} className="block w-full h-full">
              <img
                alt={p.name}
                className="w-full h-full object-cover transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] [@media(hover:hover)]:group-hover:scale-105"
                src={p.images[0] ?? ""}
                loading="lazy"
              />
            </Link>
            <div className="absolute inset-0 bg-black/10 opacity-0 [@media(hover:hover)]:group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
            <div className="absolute bottom-4 left-0 right-0 px-4 flex items-center justify-between opacity-0 [@media(hover:hover)]:group-hover:opacity-100 pointer-events-none [@media(hover:hover)]:group-hover:pointer-events-auto transition-[opacity,transform] duration-300 translate-y-1 [@media(hover:hover)]:group-hover:translate-y-0">
              <Link
                href={`/product/${p.id}`}
                className="bg-primary text-on-primary font-label-sm text-label-sm uppercase tracking-widest py-2.5 px-5 active:scale-[0.97] transition-transform duration-150"
              >
                View
              </Link>
              <button
                ...
                className="bg-primary text-on-primary w-10 h-10 flex items-center justify-center hover:bg-surface hover:text-primary border border-primary transition-colors duration-150 active:scale-[0.93] transition-transform"
              >
          ...
          <Link href={`/product/${p.id}`} className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 font-body-md text-body-md">
              <span className="text-primary">{p.currency}{p.price}</span>
