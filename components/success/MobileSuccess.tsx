...
export default function MobileSuccess() {
  return (
    <main className="flex-1 w-full flex flex-col items-center justify-center max-w-sm mx-auto mt-12 mb-8 px-margin-mobile">
      <motion.div
        {...fadeUp(0)}
        className="w-20 h-20 bg-primary rounded-full flex items-center justify-center mb-8 shadow-sm relative animate-circle-pop"
      >
      ...
      <motion.h1 {...fadeUp(0.35)} className="font-display-lg-mobile text-display-lg-mobile text-primary mb-3 text-center">
        Confirmed.
      </motion.h1>

      <motion.p {...fadeUp(0.45)} className="font-body-md text-body-md text-secondary text-center max-w-[280px] mb-10">
        Your order has been placed. We will send an email with your shipping details shortly.
      </motion.p>

      <motion.div
        {...fadeUp(0.55)}
        className="w-full border-t border-b border-surface-container py-4 flex flex-col items-center justify-center mb-10"
      >
      ...
      <motion.div
        {...fadeUp(0.65)}
        className="w-full bg-surface-container-lowest rounded-lg p-3 border border-surface-container flex items-center gap-4 mb-12 shadow-[0_2px_10px_rgba(0,0,0,0.02)]"
      >
      ...
      <motion.div {...fadeUp(0.75)} className="w-full max-w-sm mx-auto flex flex-col gap-3 mt-auto pb-4">
        <Link
          href="/"
          className="w-full h-12 bg-primary text-on-primary rounded font-label-sm text-label-sm uppercase tracking-widest flex items-center justify-center active:scale-[0.98] transition-transform duration-200"
        >
          Return to Catalog
        </Link>
        <Link
          href="#"
          className="w-full h-12 bg-transparent text-primary rounded font-label-sm text-label-sm uppercase tracking-widest flex items-center justify-center [@media(hover:hover)]:hover:underline active:scale-[0.98] transition-transform duration-200"
        >
          View Order Status
        </Link>
      </motion.div>
